...
export class PublicGroupService {
  ...
  async getAllRequestJoinGroup(groupId: Types.ObjectId, userId): Promise<{
    ...
    requests: { _id: string; sender: { firstName: string; lastName: string; avatar?: string } }[];
  }> {
    ...
    // Check if introduction exists and if visibility is private
    if (group.introduction && group.introduction.visibility === 'private') {
    ...
    const result = {
      ...
      requests: requests.map(request => ({
        _id: request._id.toString(), // Add the request ID
        sender: {
