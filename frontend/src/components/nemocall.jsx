...
export default function Call({ onClose, isOpen, targetUserIds, status }) {
    ...
    useEffect(() => {
        ...
        socket.on("offer", async ({ from, sdp }) => {
            console.log(`[Offer Handler] Received offer from ${from}. Local User: ${userId}. Current Status: ${status}.`);
            const initialTargets = targetUserIds ? targetUserIds.split(",").map(id => id.trim()) : [];
            const amICallerToThisPeer = initialTargets.includes(from);
            console.log(`[Offer Handler] Checking role: Is Local User (${userId}) the caller to ${from}? ${amICallerToThisPeer}. Status is: ${status}`);
            // === KẾT THÚC LOGGING THÊM VÀO ===
        
            if (status === 'calling' && amICallerToThisPeer) {
                 // === LOG BÊN TRONG IF ĐỂ XÁC NHẬN ===
                 console.log(`%c[Offer Handler] ROLE CHECK SUCCESS: Ignoring offer from <span class="math-inline">\{from\} because I am the caller \(status\=</span>{status}).`, 'color: orange; font-weight: bold;');
                 // === KẾT THÚC LOG BÊN TRONG IF ===
                 return;
            }
            if (!isStreamReady) {
                return;
        ...
        socket.on("answer", async ({ from, sdp }) => {
            console.log(`[Answer Handler] Received answer from ${from}. Local User ID: ${userId}`);
        
            // Kiểm tra answer từ chính mình
            if (from === userId) {
                console.warn(`[Answer Handler] Ignorning 'answer' event potentially from self (${from}).`);
                return;
            }
        
            const pc = peerConnections.current[from];
            if (!pc) {
                 console.warn(`[Answer Handler] PeerConnection for ${from} not found when receiving answer.`);
                 return;
            }
            console.log(`[Answer Handler] Found PC for ${from}. Current signalingState: ${pc.signalingState}`); // Log trạng thái hiện tại
        
            // Kiểm tra xem có localDescription (offer) không (để chắc chắn đây là Caller)
            if (!pc.localDescription || pc.localDescription.type !== 'offer') {
                console.warn(`[Answer Handler] Instance (${userId}) received an answer from ${from}, but doesn't seem to be the caller (no localDescription or it's not type 'offer'). LocalDesc:`, pc.localDescription);
                return;
            }
        
            // Kiểm tra remote description đã tồn tại chưa
            if (pc.remoteDescription) {
                console.warn(`⚠️ [Answer Handler] Remote description already exists for ${from}. Ignoring new answer. SignalingState: ${pc.signalingState}. Existing remoteDesc:`, pc.remoteDescription);
                return;
            }
        
            try {
                // === LOG TRƯỚC KHI SET ===
                console.log(`[Answer Handler] Peer ${from}. Attempting setRemoteDescription(answer). Current signalingState: ${pc.signalingState}`);
                // === KẾT THÚC LOG ===
                await pc.setRemoteDescription(new RTCSessionDescription(sdp));
                // === LOG SAU KHI SET THÀNH CÔNG ===
                console.log(`✅ [Answer Handler] Peer ${from}. setRemoteDescription(answer) SUCCESS. New signalingState: ${pc.signalingState}`);
                // === KẾT THÚC LOG ===
        
                // Xử lý ICE candidates đã lưu trữ
                if (iceCandidatesBuffer.current[from]) {
                    console.log(`[Answer Handler] Peer ${from}. Processing ${iceCandidatesBuffer.current[from].length} buffered ICE candidates.`);
                    for (const candidate of iceCandidatesBuffer.current[from]) {
                        try {
                            // Thêm log trước khi add ICE
                            console.log(`[Answer Handler] Peer ${from}. Attempting to add buffered ICE candidate. Current signalingState: ${pc.signalingState}`);
                            await pc.addIceCandidate(new RTCIceCandidate(candidate));
                            console.log(`❄️ [Answer Handler] Peer ${from}. Added buffered ICE candidate. New signalingState: ${pc.signalingState}`);
                        } catch (iceError) {
                             console.error(`❌ [Answer Handler] Peer ${from}. Error adding buffered ICE candidate:`, iceError, `. SignalingState: ${pc.signalingState}`);
                        }
                    }
                    delete iceCandidatesBuffer.current[from];
                }
                 ...
                 // === LOG KHI SET LỖI ===
                 console.error(`❌ [Answer Handler] Peer ${from}. setRemoteDescription(answer) FAILED. Error: ${error}. SignalingState before error: ${pc.signalingState}`);
                 // === KẾT THÚC LOG ===
                 cleanupPeer(from);
            }
        });
