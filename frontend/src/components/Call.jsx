...
import { PhoneXMarkIcon, XMarkIcon, PhoneIcon } from "@heroicons/react/16/solid";
...
export default function Call() {
    ...
    const connectSocket = () => {
        if (!authToken.getToken()) return alert("Vui lòng nhập token");
        console.log("🔌 [Socket] Bắt đầu kết nối với token:", authToken.getToken());
    ...
    return (
        <>
            ...
            <div className="bg-sky-600 text-white w-screen h-screen"
                id="remote-videos"
            ></div>
            <div>
                <video
            ...
            <div>
                ...
                {callStatus === "idle" && (
                    <div className="absolute bottom-5 left-1/2 transform -translate-x-1/2  flex gap-14">
                        ...
                        >
                            <PhoneIcon className="h-14 w-14 cursor-pointer bg-green-500 rounded-full text-white p-1" />
                        </button>
                    </div>
                )}
            </div>
